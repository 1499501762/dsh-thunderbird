...
- **P1 的导出路由要重启 DSH 才生效**：它是宿主改动，而宿主的模块是按 URL 缓存的。
  ...
  **新起一份宿主**验证（8 条断言：任务跑到终态、一封一个文件、**每个文件恰好一个 `#`**、
  ...
  测宿主改动的办法。
- **探针留下的 3 个 knowledge 节点还在用户记忆里**：验证 `index_doc` 通路时索引了
  3 个测试 md。想删的时候 `mdcg_forget` 两次都因没人在 8 秒内确认而被自动拒绝
  （超时不是用户否决），而重跑 `index_doc` 的 prune 只清「重新派生过的文件」
  （`touched_files: 0`），所以没清掉。**没有绕过这个拒绝。** 用灵枢自己的工具删即可。
